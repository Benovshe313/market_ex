namespace trouble2
{
    internal class Program
    {
        ...
        Register:
            ...
            try
            {
                Models.UserManager.CreateAccount(firstName!, lastName!, birth!, email!.ToLower().Trim(), password!);
            }
            catch (Exception ex)
            {
        ...
        public static bool LoginPage()
        {
            while (true)
            {
                ...
                if (Models.UserManager.Login(loginEmail!, loginPassword!))
                {
                    Console.WriteLine("Login successful!");
                    if (Models.UserManager.User is not null)
                    {
                        Console.WriteLine($"Welcome {Models.UserManager.User.Name}!");
                        Thread.Sleep(2000);
